...
class PandasAPISparkDataSet(AbstractVersionedDataSet):
    ...
    def _load(self) -> DataFrame:
        ...
        return ps.DataFrame(
            read_obj.load(load_path, self._file_format, **self._load_args)
        )

    def _save(self, data: DataFrame) -> None:
        save_path = _strip_dbfs_prefix(self._fs_prefix + str(self._get_save_path()))
        data.to_spark().write.save(save_path, self._file_format, **self._save_args)
